...
This document owns production deployment. Local development lives in [GETTING_STARTED.md](getting-started.md).
...
- optional `DELIVERY_BASE_URL`, `DELIVERY_ADMIN_KEY` if using megabat-native Telegram integration routes
...
- `megabat` is created by the Postgres container via `POSTGRES_DB`
- `megabat_delivery` is created on first boot by the init script in `docker/postgres/init`
...
- [GETTING_STARTED.md](getting-started.md) for local setup
- [API.md](../reference/api.md) for health and webhook routes
- [TELEGRAM_DELIVERY.md](../integrations/telegram-delivery.md) for delivery contract details
