...
### Decision 19: megabat-native Telegram Status Endpoints

- problem: the web app should not need to speak directly to delivery or know the raw `app_user_id` wiring
- fix: expose Telegram link status and token-link routes through megabat, backed by delivery internal endpoints
...
- [DSL.md](../product/dsl.md) for the user-facing signal contract
- [SOURCES.md](sources.md) for source-family capability and extension rules
- [ISSUE_NO_TIME_TRAVEL.md](no-time-travel.md) for the Envio/RPC split
